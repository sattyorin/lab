...
def get_body(name : str, modules : str, position_z : float) -> str:
  return f'<body name="{name}" pos="0 0 {position_z}">\n' + modules + '</body>\n'

def get_module(module_id : int, position_x : float, position_y : float, size_x : float = 0.05, size_y : float = 0.05, size_z : float = 0.4) -> str:
  return f'<body name="module{module_id}" pos="{position_x} {position_y} 0">\n' + f'<geom name="box{module_id}" type="box" size="{size_x} {size_y} {size_z}" />\n' + f'<joint name="joint{module_id}" type="slide" pos="0 0 0" axis="0 0 1" range="-0.01 0" damping="10000" />\n' + '</body>\n'

def get_actuator(motors : str) -> str:
  return '<actuator>\n' + motors + '</actuator>\n'

def get_motor(motor_id : int) -> str:
  return f'<motor name="a{motor_id}" gear="20000" joint="joint{motor_id}" />\n'

def parse_args() -> argparse.Namespace:
  parser = argparse.ArgumentParser()
  parser.add_argument("-p", "--path", default="envs/linear_actuator_array/xmls/linear_actuator_array-v0.xml")
